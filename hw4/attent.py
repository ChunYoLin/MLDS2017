...
def read_file():
    ...
    return input_raw_list, target_raw_list, total_words, max_len+2
...
class Seq2SeqModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        weight_a = tf.get_variable("weight_a", [size, sent_len], \
                                            dtype=data_type(0))
        ...
        with tf.variable_scope("Attention"):
            ...
            for time_step in range(sent_len):
                ...
                encode_output, state = cell(inputs[:, time_step, :], state)
                encode_info_list.append(encode_output)
            encode_info = tf.reshape(tf.concat(axis=1, values=encode_info_list), \
                                        [-1, sent_len, size])
            ...
            for time_step in range(sent_len-1):
                ...
                (decode_output, state) = \
                                cell(response, state)
                _hidden_t = tf.reshape(decode_output, [-1, size])
                _location = tf.nn.softmax( tf.matmul(_hidden_t, weight_a) )
                _vector_c_list = []
                for _b in range(batch_size):
                    _weight = tf.reshape( _location[_b, :], [1, sent_len] )
                    _info = tf.reshape( encode_info[_b, :, :], [sent_len, size] )
...
def run_predict(session, model, eval_op=None, verbose=False):
    ...
    try:
        _pos_x = np.where(y == 2)[0][0]
        _pos_y = np.where(y == 2)[0][0]
        _pos_pred = np.where(pred_words == 2)[0][0]
    except IndexError:
        _pos_x = 3
        _pos_y = 3
        _pos_prob = 3
...
def main(_):
    ...
    print "\n--- Read Train Data ---"
    train_raw_input, train_raw_target, total_words, sent_len = read_file()
    print "\n--- Build Words Dict ---"
    ...
    print "\n--- Data Sample ---"
    for i in range(5):
        print "Input: _BOS %s" % (' '.join(train_raw_input[i]))
        print "Target: _BOS %s" % (' '.join(train_raw_target[i]))
    #'''
                ...
                for epoch_step in range(config.max_max_epoch):
                    ...
                    if ((epoch_step+1) % 100) == 0:
                        ...
                        for i in range(valid_config.data_size):
                            x, y, pred = run_predict(session, mvalid)
