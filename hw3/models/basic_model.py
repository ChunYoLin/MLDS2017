...
class GAN(object):
    ...
    def build_model(self):
        ...
        self.mismatch_sent = self.sent_dim_reducer(self.mismatch_sent_highdim,
                reuse=True)
        ...
        self.Sr, self.Sr_logits = self.discriminator(match_sent, real_image,
                reuse=False)
        Sr, Sr_logits = self.Sr, self.Sr_logits
        #  real image, wrong text
        self.Sw, self.Sw_logits = self.discriminator(mismatch_sent, real_image,
                reuse=True)
        Sw, Sw_logits = self.Sw, self.Sw_logits
        #  fake image, right text
        self.Sf, self.Sf_logits = self.discriminator(match_sent, fake_image,
                reuse=True)
        Sf, Sf_logits = self.Sf, self.Sf_logits
        #  loss of discriminator
        self.d_loss_real = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.ones_like(Sr), 
                logits=Sr_logits))
        self.d_loss_fake = tf.reduce_mean(
            (tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.zeros_like(Sw),
                logits=Sw_logits)
            + tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.zeros_like(Sf),
                logits=Sf_logits)) / 2.
            )
    ...
    def train(self):
        ...
        d_optim = tf.train.AdamOptimizer(learning_rate).minimize(self.d_loss,
                var_list=self.d_vars)
        g_optim = tf.train.AdamOptimizer(learning_rate).minimize(self.g_loss,
    ...
    def sent_dim_reducer(self, sent, reuse=False):
        with tf.variable_scope("sent_dim_reducer") as scope:
            ...
            w = tf.get_variable("g_sent_reduce_w", 
                    [self.orig_sent_size, self.sent_size], tf.float32,
                    tf.random_normal_initializer(stddev=0.01))
            b = tf.get_variable("g_sent_reduce_b", [self.sent_size], tf.float32,
